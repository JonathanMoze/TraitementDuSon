...
runPlugin(LADSPA_Handle Instance,
  ...
  int delay_index = pfParam1*sr;


  for(int i=0; i<SampleCount; i++){
    
    pfOutput1[i] = (1-pfParam2)*pfInput1[i] + pfParam2 * save1[((sr)+i)-delay_index];
    pfOutput2[i] = (1-pfParam2)*pfInput2[i] + pfParam2 * save2[((sr)+i)-delay_index];
  }

  for(int i = 0; i<SampleCount; i++){
    save1[(sr-SampleCount)+i] = pfInput1[i];
    save2[(sr-SampleCount)+i] = pfInput2[i];
  }

  for(int i=0; i<sr-SampleCount; i++){
