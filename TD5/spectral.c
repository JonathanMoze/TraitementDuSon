...
int isEnoughEnergy(double* amp_buff){
	double energie = 0;
	for(int i=0; i<FRAME_SIZE; i++){
		energie += pow(amp_buff[i], 2);
	}
	energie *= (1.0/FRAME_SIZE);
	return (energie > ENERGY_THRESHOLD);
}
...
{	char 		*progname, *infilename;
	...
	while (read_samples (infile, new_buffer, sfinfo.channels)==1)
	{
		...
		/* Recherche de la freq maximum et de son amplitude (normalisée) */
		if(isEnoughEnergy(buffer_amp)){ 
			if(!soundBefore){
				int index_max_f = 0;
				for(int f=1; f<(FFT_SIZE/2)-1; f++){
					...
					if(buffer_amp[f-1] <= buffer_amp[f] && buffer_amp[f+1] < buffer_amp[f]){
						switch(index_max_f){
							case 0:
								f_max[0] = abs[f];
								amp_max[0] = buffer_amp[f];
								index_max_f++;
								break;
							case 1:
								f_max[1] = abs[f];
								amp_max[1] = buffer_amp[f];
								index_max_f++;
								break;
							case 2:
								for(int j=0; j<2; j++){
									if(buffer_amp[f] > amp_max[j]){
										f_max[j] = abs[f];
										amp_max[j] = buffer_amp[f];
										break;
									}
								}
						}
				...
				printf("Son numéro %d, Frequence 1:%d, Frequence2:%d\n", numeroSon, f_max[0], f_max[1]);
